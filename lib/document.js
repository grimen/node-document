...
function Document (type) {
  ...
  var self = this, args = fun(arguments), schema, storage, callback;
...
// TODO: Document.Storage = Dcoument.DefaultStorage;
...
Document.Model = function(attributes) {
  ...
  this.validator = new Document.Validator();
...
Document.Model.prototype.__defineGetter__('changed', function() {
  var _changed = (this.changes.keys().length > 0);
  return _changed;
});
...
Document.Model.prototype.set = function() {
  ...
  if (args.length === 1 && (!args[0] || Object.isObject(args[0]))) {
...
Document.Model.prototype.del = function() {
  ...
  if (args.length === 1) {
    var _keys =  Object.isArray(args[0]) ? args[0] : [args[0]];
...
Document.Model.prototype.attr = function() {
  ...
  if ( args.length === 0 || (args.length === 1 && Object.isString(args[0])) ) {
    var _key = args[0];
    return args.length === 1 ? self.get(_key) : self.get();

  } else if ( args.length === 2 || (args.length === 1 && Object.isObject(args[0])) ) {
...
Document.Model.prototype.clear = function(what) {
  ...
  if (args.length === 0 || what === 'attributes') {
    self.attributes = Object.extended({});
  }

  if (args.length === 0 || what === 'changes') {
    self.changes = Object.extended({});
  }

  if (args.length === 0 || what === 'errors') {
    self.errors = null;
  }
...
Document.Model.prototype.validate = function() {
  ...
  self.validator.validate(self.attributes, self.klass.schema, options, function(err) {
    var errors = err || null;
    ...
    callback.call(this, errors, valid);
  });
