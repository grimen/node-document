...
function AmazonS3 () {
  ...
  var options = {
    ...
    port: (endpoint.port || '').isBlank() ? undefined : parseInt(endpoint.port, 10),
  ...
  self.options.client.key = self.options.username;
  self.options.client.secret = self.options.password;
  self.options.client.secure = (self.options.protocol === 'https:');
  self.options.client.bucket = self.options.db;
  // self.options.client.region = 'us-standard'; // TODO: Try extract from '*.s3-<region>.amasonaws.com'
  // self.options.client.endpoint = self.options.hostname; // TODO: Try extract from '<bucket>.s3-*.amasonaws.com'
  self.options.client.port = self.options.port || (self.options.client.secret ? 443 : 80);
}
...
AmazonS3.prototype.key = function(key) {
  var self = this;
  return '/' + key + self.options.settings.extension;
};
