...
Storage.prototype._exists = function(args, execute) {
  ...
  self.command('exists', args, 3, function(keys, options, callback) {
    keys = Array.create(keys).map(function(k) { return self.key(k); });
    keys = keys.map(function(k) { return k.key ? k.key : k; });
    ...
    var next = function(key, error, result, response) {
      ...
      if (Object.keys(res).length === keys.length) {
        ...
        _done(errors, results, responses);
      }
    };

    var _done = function(errors, results, responses) {
      results = results.map(function(result) {
        return result;
      });
    ...
    execute(keys, options, _done, next);
  });
};
