...
function ElasticSearchStorage (options) {
  options = Object.merge(options, ElasticSearchStorage.options);
  ...
  var endpoint_url = process.env.ELASTICSEARCH_INDEX_URL || this.klass.url,
  ...
  var defaults = Object.extended({
    ...
    auth: endpoint.auth,
    // BUG: protocol: endpoint.protocol,
    settings: {},
    db: endpoint.pathname.replace(/^\//, ''),
  });
  options = defaults.merge(options);

  this.client = new elastical.Client('localhost', options);
