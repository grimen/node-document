...
Redis.prototype.connect = function() {
  ...
  self._connect(function() {
    self.client = require('redis').createClient(self.options.client.port, self.options.client.hostname, self.options.client);

    if (self.options.client.password) {
      self.client.auth(self.options.client.password, function(err, reply) {
...
Redis.prototype.set = function() {
  ...
  self._set(arguments, function(key_values, options, done, next) {
    var commands = Object.keys(key_values).map(function(k, v) { return ['set', k, key_values[k]]; });
