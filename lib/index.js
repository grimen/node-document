...
function Document (type) {
  ...
  // REFACTOR
  klass.meta_field = function(field_name) {
    var self = this;
    return function() {
      var prefixed_field_name = (self.meta._prefix || '') + field_name;
      return prefixed_field_name;
    };
  };
  klass.meta = {};
  klass.meta._prefix = '_';

  // TODO: Make meta fields "pluggable" in some cleaner way (API); see middelware/hooks in TODO.
  klass.meta.__defineGetter__('id', klass.meta_field('id'));
  // REVIEW: klass.meta.__defineGetter__('timestamp', klass.meta_field('timestamp'));
  // REVIEW: klass.meta.__defineGetter__('type', klass.meta_field('type'));
